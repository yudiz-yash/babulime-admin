...
const ICON_OPTIONS = ['Award', 'ShieldCheck', 'CheckSquare', 'FileText', 'Shield', 'Star', 'CheckCircle', 'Zap'];

export default function CompliancePage() {
  const [data, setData] = useState({ heading: '', description: '', items: [] });
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    api.get('/settings/compliance')
  ...
  const addItem = () => setData(d => ({ ...d, items: [...d.items, { icon: 'Award', title: '', desc: '' }] }));
  const removeItem = (i) => setData(d => ({ ...d, items: d.items.filter((_, idx) => idx !== i) }));

  const save = async () => {
    setSaving(true);
    try {
      const res = await api.put('/settings/compliance', data);
      setData(res.data);
      toast.success('Compliance section saved!');
    } catch {
  ...
  return (
    <div className="max-w-3xl space-y-6">
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1">Edit compliance cards section</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-6 space-y-5">
        <div className="flex items-center justify-between">
          <h3 className="font-semibold text-gray-800 text-sm">Compliance Cards</h3>
          ...
          <div key={i} className="border border-gray-100 rounded-xl p-4 space-y-3">
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              <div>
                ...
                <select value={item.icon || 'Award'} onChange={e => updateItem(i, 'icon', e.target.value)}
